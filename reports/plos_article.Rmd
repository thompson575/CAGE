...
```{r}
plot_in_out_loss(r$vpcaSelectedDF) +
...
```{r}
plot_in_out_loss(r$vpcaOrderedDF) +
...
```{r}
plot_in_out_loss(r$cpcaSelectedDF) +
...
```{r}
plot_in_out_loss(r$cpcaOrderedDF) +
...
list( probe  = r$probeSelectedDF,
      PCsel  = r$vpcaSelectedDF,
      PCord  = r$vpcaOrderedDF,
      sPCsel = r$cpcaSelectedDF,
      sPCord = r$cpcaOrderedDF)  %>%
  plot_method_comparison(inloss) +
  ggtitle("Features 1 to 50") -> p1

list( probe  = r$probeSelectedDF     %>% slice(1:8),
      PCsel  = r$vpcaSelectedDF       %>% slice(1:8),
      PCord  = r$vpcaOrderedDF        %>% slice(1:8),
      sPCsel = r$cpcaSelectedDF %>% slice(1:8),
      sPCord = r$cpcaOrderedDF  %>% slice(1:8))  %>%
  plot_method_comparison(inloss) +
